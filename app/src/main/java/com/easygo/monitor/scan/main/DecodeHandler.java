...
final class DecodeHandler extends Handler {
  ...
  private void decode(byte[] data, int width, int height) {
    ...
    PlanarYUVLuminanceSource source = mCaptureFragment.getCameraManager().buildLuminanceSource(data, width, height);
		if (source != null) {
			try {
    ...
    Handler handler = mCaptureFragment.getHandler();
    if (rawResult != null) {
      // Don't log the barcode contents for security.
      long end = System.currentTimeMillis();
      EZLog.debugLog(TAG, "Found barcode in " + (end - start) + " ms");
      if (handler != null) {
