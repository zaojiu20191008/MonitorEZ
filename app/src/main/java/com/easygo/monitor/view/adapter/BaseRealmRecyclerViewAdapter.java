...
package com.easygo.monitor.view.adapter;
 ...
 */
public abstract class BaseRealmRecyclerViewAdapter<T extends RealmModel, VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> {
    ...
    @Deprecated
    public BaseRealmRecyclerViewAdapter(@NonNull Context context, @Nullable OrderedRealmCollection<T> data, boolean autoUpdate) {
    ...
    public BaseRealmRecyclerViewAdapter(@Nullable OrderedRealmCollection<T> data, boolean autoUpdate) {
        ...
        this.listener = hasAutoUpdates ? new RealmChangeListener() {
            @Override
            public void onChange(Object results) {
                Log.d("adapter","notifyDataSetChanged   "+ BaseRealmRecyclerViewAdapter.this.toString());
                notifyDataSetChanged();
            }
        } : null;
    }
